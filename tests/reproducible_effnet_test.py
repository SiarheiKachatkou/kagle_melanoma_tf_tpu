...
class ReproTest(unittest.TestCase):
    ...
    def test_repro_create_model(self):
        ...
        cfg = config(model_fn_str="efficientnet.tfkeras.EfficientNetB0(weights='imagenet', include_top=False)",
                     l2_penalty=0, image_height=img_size, use_meta=0, epochs_fine_tune=0, dropout_rate=0.1)
        metrics = []
        optimizer = tf.keras.optimizers.Adam(learning_rate=0.01)
        model = create_model(cfg, metrics, optimizer, fine_tune_last=None, backbone_trainable=False)
        img = np.random.uniform(low=0.0, high=1.0, size=(1, img_size, img_size, 3)).astype(np.float32)
        ...
        output2 = model.predict(img)
        print(output2)
        self.assertAlmostEquals(output1[0, 0], output2[0, 0], delta=1e-6)
